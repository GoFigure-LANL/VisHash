...
class ImageSignature(object):
    ...
    crop_percentiles (Optional[Tuple[int]]): lower and upper bounds when
        considering how much variance to keep in the image (default (1, 99))

    patch_width (Optional[int]): width in pixels of sample region. If none, the
        patch width is min(width, height) of the image divided by n
        (default None)

    n_levels (Optional[int]): number of positive and negative groups to
        stratify neighbor differences into. n = 3 -> [-3, -2, -1, 0, 1, 2, 3]
        (default 3)

    proportional_patch (Optional[boolean]): Automatically adjust patch height
        so that patches have the same aspect ratio as the original image,
        otherwise the patch height is equal to the patch width (default True)

    threshold (Optional[float]): threshold between 0 and 1 for considering
        difference of neighboring regions to be identical in mean gray value,
        or None to automatically calculate this threshold based on patch size
        (default None)
    """
    ...
    def __init__(
        self,
        n=9,
        crop_percentiles=(1, 99),
        patch_width=None,
        n_levels=3,
        proportional_patch=True,
        threshold=None,
    ):
        # check inputs
        assert (
        ...
        if crop_percentiles is not None:
            ...
            ), "Upper crop_percentile limit must be greater than lower limit."
        ...
        if patch_width is not None:
            assert (
                type(patch_width) is int
            ), "patch_width should be an integer >= 1, or None"
            assert patch_width >= 1, (
                "patch_width should be greater than 0 (%r given)" % patch_width
            )
        self.patch_width = patch_width

        assert type(proportional_patch) is bool, (
            "proportional_patch should be boolean (%r given)" % proportional_patch
        )
    ...
    def generate_signature(self, path_or_image, bytestream=False):
        ...
        Returns:
            The image signature: A numpy vector of length 2n(n-1) for an nxn
                grid
        """
    ...
    def convert_color(img):
        """
        From a numpy array (dtype=np.uint8), returns a 2-d numpy array of the
        grayscale image.
        """
        if len(img.shape) > 2:
            if img.shape[2] == 4:
                # Check if non-transparency is mostly white
                # (then make background black)
                alpha = img[:, :, 3]
                inds = alpha > 0
    ...
    def preprocess_image(self, image_or_path, bytestream=False):
        ...
        Args:
            ...
            bytestream (Optional[boolean]): will the image be passed as raw
                bytes? That is, is the 'path_or_image' argument an in-memory
                image? (default False)
        Returns:
        ...
        if bytestream:
            self.im_array = self.preprocess_bytestream(image_or_path)
        elif type(image_or_path) is str:
            self.im_array = self.preprocess_path(image_or_path)
        elif type(image_or_path) is bytes:
            self.im_array = self.preprocess_bytes(image_or_path)
        elif type(image_or_path) is np.ndarray:
            self.im_array = self.preprocess_ndarray(image_or_path)
        else:
            raise TypeError("Path or image required.")

        return self.im_array

    def preprocess_bytestream(self, image):
        try:
            img = Image.open(BytesIO(image))
        except IOError:
            # could be an svg, attempt to convert
            try:
                img = Image.open(BytesIO(svg2png(image)))
    ...
    def preprocess_bytes(self, image):
        try:
    ...
    def preprocess_ndarray(self, image):
    ...
    def crop_image(self):
        ...
        upper_column_limit = np.searchsorted(
            cw, np.percentile(cw, self.upper_percentile), side="left"
        )
        lower_column_limit = np.searchsorted(
            cw, np.percentile(cw, self.lower_percentile), side="right"
        )
        upper_row_limit = np.searchsorted(
            rw, np.percentile(rw, self.upper_percentile), side="left"
        )
        lower_row_limit = np.searchsorted(
            rw, np.percentile(rw, self.lower_percentile), side="right"
        )
    ...
    def compute_grid_points(self):
        ...
        Returns:
            tuple of arrays indicating the vertical and horizontal locations of
                the grid points
        ...
        # Calculate the upper limit of the patch size given n and dimensions of
        # cropped image
        ...
        self.x_coords = np.linspace(
            window[0][0] + width_max / 2,
            window[0][1] - width_max / 2,
            self.n,
            dtype=int,
        )
        self.y_coords = np.linspace(
            window[1][0] + height_max / 2,
            window[1][1] - height_max / 2,
            self.n,
            dtype=int,
        )
    ...
    def compute_gray_means(self):
        ...
        Returns:
            an n x n array of average grayscale around the gridpoint,
        ...
        if self.patch_width is None:
            if self.proportional_patch:
                ...
                self.patch_width = (
                    self.width_max
                )  # calculated when assigning grid points
                self.patch_height = self.height_max
            else:
                # Patch will fill one direction but not overflow the other,
                # the patch is square
                self.patch_width = np.min([self.width_max, self.height_max])
                self.patch_height = self.patch_width
        else:
            # Note, there is currently no checking whether patch overflows the
            # image or causes the patches to overlap
            if self.proportional_patch:
        ...
        for i, x in enumerate(self.x_coords):
            ...
            for j, y in enumerate(self.y_coords):
                ...
                upper_y_lim = int(
                    min([lower_y_lim + self.patch_height, image.shape[1]])
                )

                avg_gray[i, j] = np.mean(
    ...
    def compute_differentials(self):
        ...
        The values are the differences between neighboring patches,
        in this order:
        left: n*(n-1) values
        up:   n*(n-1) values
    ...
    def threshold_diffs(self):
        """
        Difference vector is thresholded so that near-zero values are set to
        zero. Fixed threshold can be specified, or if none, calculate threshold
        based on size of patch
        """

        # set very close values as equivalent
        if self.threshold is None:
            # Identical tolerance is 1 / (sqrt(6 * w * h)
            # where w and h are the patch_width and patch_height
            self.threshold = 1.0 / (
                8 * np.sqrt(6) * np.sqrt(self.patch_width) * np.sqrt(self.patch_height)
            )

        mask = np.abs(self.diff_vec) < self.threshold
        self.diff_vec[mask] = 0.0
    ...
    def bin_diffs(self, wbg_version=False):
        ...
        if wbg_version:
            positive_cutoffs = np.percentile(
                self.diff_vec[self.diff_vec > 0.0],
        ...
        # This preserves the bin boundaries for comparing flipped images where
        # the differences will have opposite sign.
        abs_cutoffs = np.percentile(
            np.abs(self.diff_vec), np.linspace(0, 100, self.n_levels + 1)
        )
