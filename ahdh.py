...
class AdaptiveHistogram(object):
    ...
    Args:
        ...
        n_nonquant_levels (Optional[int]): lowest level which will
                          use quantized relative levels (default 2)
    """

    def __init__(
        self, n_levels=10, n_nonquant_levels=2, output_dir="./", save_all_images=False
    ):
        """Initialize the signature generator"""
        self.im_array = None

        assert type(n_levels) is int, "n_levels should be an integer > 0"
        assert n_levels > 0, "n_levels must be non-negative (%r given)" % n_levels
        self.n_levels = n_levels

        assert type(n_nonquant_levels) is int, "n_nonquant_levels should be an integer"
        assert n_nonquant_levels >= 0, (
            "n_nonquant_levels should be at least 0 (%r given)" % n_nonquant_levels
        )
    ...
    def generate_signature(self, fname):
        ...
        they are not included in this code. The returned image signature
        contains
        (4/3)*(4^(n_nonquant_levels) - 1) + 15*(n_levels - n_nonquant_levels)
        real values.
        ...
        for level in range(self.n_levels):
            regions, n_pixels, areas = self.ahdh_extraction(
                regions, n_pixels, areas, level
            )
    ...
    def load_img_as_binary(self, fname):
        """
        Read in the given image and convert to binary. Return ndarray. If
        read fails, return None value.
        """
        ...
        try:
            ...
            print("Image file " + fname + " not found.")
        except Exception as e:
            print("Failed to open image " + fname + ": " + e)
            return
    ...
    def save_image(self, prefix, bin_array):
        """
        Takes a binary array, inverts black/white, then saves to the
        given filename.
        """
        filename = self.output_dir + "/" + prefix + self.im_name + ".png"
        imsave(
            filename,
            skimage.img_as_ubyte(np.logical_not(bin_array)),
            check_contrast=False,
        )
        return

    def save_partition_images(self, prefix, regions, centroids):
        # Could be a better way of doing this - probably lines of subregion
        # edges, and accumulate on the full image rather than save just the
        # region.
        ...
        for i, region in enumerate(regions):
            ...
            if region.size < 1:
                continue
            # background
            im_color = np.ones(region.shape, dtype=np.uint8) * 255
            # foreground
            im_color[region] = 0
    ...
    def ahdh_extraction(self, regions, n_pixels, areas, level):
        """
        For the given level, calculate the feature vector of the given regions.
        Returns a vector of length 4^(level + 1) of densities, unless this is a
        quantized relative density level, in which case the vector is of length
        15 representing a histogram.

        Args:
            regions (list of length 4^level of variable-sized ndarrays):
                Binary images
            n_pixels (4^level x 1 ndarray): Number of foreground pixels per
                region
            areas (4^level x 1 ndarray or None): Number of all pixels per
                region, or None if not yet calculated
            level (int): Which layer of the hierarchy, starts at 0

        Returns:
            features (4^(level+1)x1 ndarray or 15x1 ndarray): Density features
                for this level
        """
    ...
    def get_subregions(self, region):
        """
        For the given region, find the centroid and return the slices
        associated with the four new regions based on the centroid.
        """
    ...
    def count_foreground(self, subregions):
        """
        For the given list of subregions, count the number of foreground
        pixels.
    ...
    def calc_densities(self, n_subregion, n_region):
        """
        Calculate the [subregion number of foreground pixels] / [region number
        of foreground pixels] for each subregion. In the case that the region
        has 0 foreground pixels, the density for all of the associated
        subregions is 1.

        Args
            n_subregion (4^level x 4 ndarray): Number of foreground pixels per
                subregion, where each row is a set of subregions sharing the
                same parent region
            n_region (4^level x 1 ndarray): Number of foreground pixels per
                region
    ...
    def count_codes(self, rel_densities):
        """
        Each row of rel_densities is treated as a codeword. The frequency of
        codewords is tabulated (with 15 possible codewords, because 0b0000 is
        not possible). The returned code_count values are real values between
        0 and 1.

        Args
            rel_densities (binary 4^levelx4 ndarray): The quantized relative
                densities

        Return
            code_count (real 15x1 ndarray): Histogram of code [0b0001 - 0b1111]
                frequency
        """
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If given, process only listed images ",
            "(1st column of text file from image_path)",
        ),
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "If enabled, all intermediate processing steps will be saved ",
            "as images to OUTPUT_DIR",
        ),
    )
    parser.add_argument(
        "--output_dir",
...
def main():
    ...
    if args.image_list is None:
        ...
        with open(args.image_list, encoding="utf-8") as f_in:
            img_fnames = [args.image_path + "/" + line.split()[0] for line in f_in]

    im_sig = AdaptiveHistogram(
        args.n_levels, args.n_nonquant_levels, save_all_images=args.save_all_images
    )

    # Calculation is slow, so save each hash as it calculated
    with open(args.output_dir + "/" + args.output + ".csv", "w", encoding="utf-8") as f:
