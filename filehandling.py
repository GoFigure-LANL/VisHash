...
def read_filenames(postfix, path):
    fnm = []
    with open(path + '/filenames_' + postfix + '.csv') as infile:
        for line in infile:
            words = line.split(', ')
...
def read_filename_lookup(postfix, path):
    with open(path + '/filenames_' + postfix + '.json') as infile:
        fnm_lookup = json.load(infile)
    return fnm_lookup


def write_filename_lookup(postfix, path, data):
    with open(path + '/filenames_' + postfix + '.json', 'w') as outfile:
...
def read_filenames_from_file(pth, input_file, N):
    """
    Read given input_file to gather names of image files. Preprend pth to each filename from
    input_file.
...
def read_mindists(postfix, path):
    with open(path + '/mindist_' + postfix + '.csv') as infile:
...
def save_matchlist(midx, dst, filenames, postfix):
    """
    For all images in match list, write a file with the filenames of the images and the
    distance. The output is written in the same order as the given midx.
    """
    # Check that there is actually a list to construct
    if isinstance(midx[0], (list, np.ndarray)):
        n_pairs = len(midx[0])
        file_pairs = [(filenames[midx[0][i]], filenames[midx[1][i]]) for 
                      i in range(n_pairs)]
...
def read_matches(postfix, path):
    with open(path + '/matches_' + postfix + '.csv') as infile:
        mindists = {'k':[], 'fileA':[], 'fileB':[], 'dist':[]}
        reader = csv.DictReader(infile)
        for row in reader:
            mindists['k'].append(row['order'])
...
def write_matches(postfix, path, file_pairs, dist_list):
    with open(path + '/matches_' + postfix + '.csv', 'w') as outfile:
        fieldnames = ['order', 'fileA', 'fileB', 'dist']
        writer = csv.DictWriter(outfile, fieldnames=fieldnames)
        writer.writeheader()
        for i in range(len(file_pairs)):
...
def read_topk(postfix, path):
    with open(path + '/topk_' + postfix + '.json') as infile:
...
def write_topk(postfix, path, query_dict):
    ...
    """
    with open(path + '/topk_' + postfix + '.json', 'w') as outfile:
