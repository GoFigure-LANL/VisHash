...
def get_matches_sym(dst, threshold):
    """
    Returns all matching unordered pairs below given threshold. dst assumed to be symmetric.
    """
    start = time.time()
...
def get_args():
    parser = argparse.ArgumentParser(description='Calculate distances between all-pairs of given signature list and store matches below given threshold.')
    parser.add_argument('--postfix', type=str,
                        help='filenames_[postfix].csv and signatures_[postfix].npy will be read while matches_[postfix].csv will be written')
    parser.add_argument('--path', type=str, default='./',
                        help='Path to input files [./]')
    parser.add_argument('-t', '--threshold', type=float, default=0.3,
                        help='Distance threshold, such that matches will only be stored for distance < threshold [0.3]')
