...
def compute_sigs(fnm, prfx):
    """
    Given the list of filenames, compute VisHash for each image. Two output
    files are written:
    filenames_[prfx].csv: List of filenames of images processed
    signatures_[prfx].npy: Numpy array of hashes, in the same order as the file
        names list
    """
    print(time.asctime(), " Computing signatures")
    start = time.time()
    (ftr, _) = util.compute_features(fnm)
    end = time.time()
...
def calc_distances(ftr, prfx=None):
    """
    Compute all-unordered-pairs distances on the given list of hashes (ftr). If
    prfx is given, save the full dst matrix as mat_[prfx].npy (not recommended,
    as this can be a very large file).
    """
...
def get_min_distances(dst, n_matches):
    """
    For the given distance matrix, return a tuple of two lists of indices such
    that dst[midx[0][i], midx[1][i]] are the minimum-distance values in dst,
    for i<n_matches.
...
def get_args():
    ...
    parser.add_argument(
        "--dataname",
        type=str,
        help="The name of the dataset (for storing output files)",
    )
    parser.add_argument(
        "--image_path",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "instead of processing the whole directory [None]",
        ),
    )
    parser.add_argument(
        "--num_images",
        ...
        default=None,
        help=(
            "If specified limit number of images to num_images (usually ",
    ...
    parser.add_argument(
        ...
        default=1000,
        help=(
            "If specified limit number of matches n_matches ",
